...
def training(args):
    ...
    if args.train_with_aug:
        ...
        if args.data_name == 'snli':
            total_src_list['train2'] = np.concatenate((total_src_list['train2'], np.array(aug_src_list['train2'])))
    ...
    tokenizer_name = return_model_name(args.model_type)
    ...
    for epoch in range(start_epoch + 1, args.training_num_epochs + 1):
        ...
        for i, batch_iter in enumerate(tqdm(dataloader_dict['train'], bar_format='{l_bar}{bar:30}{r_bar}{bar:-2b}')):
            ...
            # Accuracy
            acc = sum((logit.argmax(dim=1) == trg_label)) / len(trg_label)
        ...
        save_file_name = os.path.join(args.model_save_path, args.data_name, args.model_type, 
                                      f'cls_training_checkpoint_seed_{args.random_seed}_aug_{args.train_with_aug}_{args.aug_method}.pth.tar')
    ...
    for batch_iter in tqdm(dataloader_dict['test'], bar_format='{l_bar}{bar:30}{r_bar}{bar:-2b}'):
        ...
        with torch.no_grad():
            ...
            test_acc += sum((logit.argmax(dim=1) == trg_label)) / len(trg_label)
